      ...
      <div class="card">
        <div class="card-body">
          <div class="list-group list-group-flush" id="setting-list">
            ...
            <div class="mb-3" :class="hasPermissions('setting_menu_builder') ? 'active-menu' : ''"
              id="Settings.menu-builder" v-if="hasPermissions('setting_menu_builder')" @click="toggle()"><router-link
                :to="{ name: 'Settings.menu-builder' }" class="btn btn-border"><i class="fa fa-bars"
                  aria-hidden="true"></i>{{ $t('setting_sidebar.lbl_menu') }} </router-link></div>
          </div>
        </div>
      </div>
    </div>
